...
class ApplyCosmeticsServiceTest {
    ...
    private CosmeticFindApi cosmeticFindApi;
    ...
    @BeforeEach
    public void setUp() {
        cosmeticFindApi = mock(CosmeticFindApi.class);
        checkWikipediaService = mock(CheckWikipediaService.class);
        applyCosmeticsService = new ApplyCosmeticsService(cosmeticFindApi, checkWikipediaService);
    }

    @Test
    void testApplyCosmeticChanges() {
        Cosmetic cosmetic = Cosmetic.builder().start(2).text("[[Link|link]]").fix("[[link]]").build();
        when(cosmeticFindApi.findCosmetics(any(FinderPage.class)))
        ...
        verify(cosmeticFindApi).findCosmetics(page);
