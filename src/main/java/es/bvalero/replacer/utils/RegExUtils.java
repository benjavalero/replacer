...
public class RegExUtils {
    ...
    // The different exceptions in a text may overlap
    static final String REGEX_TEMPLATE_PARAM = "\\|[\\wÁáÉéÍíÓóÚúÜüÑñ\\s]+=";
    static final String REGEX_INDEX_VALUE = "\\|\\s*índice\\s*=[^}|]*";
    static final String REGEX_UNREPLACEBLE_TEMPLATE = "\\{\\{(?:ORDENAR:|DEFAULTSORT:|NF\\||[Cc]ita\\||c?[Qq]uote\\|)[^}]+}}";
    ...
    public static List<Interval> findExceptionIntervals(String text) {
        if (exceptionPatterns.isEmpty()) {
            ...
            exceptionPatterns.add(Pattern.compile(REGEX_INDEX_VALUE));
